...
void main() {
  ...
  group('HttpClient Tests', () {
    group('POST Requests', () {
      ...
      test(
        'POST request with invalid JSON response throws exception',
        () async {
          when(
            mockClient.post(
              Uri.parse('$baseUrl/test'),
    ...
    group('GET Requests', () {
      test('successful GET request returns decoded JSON', () async {
        ...
        when(
          mockClient.get(Uri.parse('$baseUrl/test')),
        ).thenAnswer((_) async => http.Response(jsonEncode(responseBody), 200));
    ...
    group('GET Bytes Requests', () {
      ...
      test(
        'GET bytes request with non-200 status code throws exception',
        () async {
          when(
            mockClient.get(Uri.parse('$baseUrl/test')),
          ).thenAnswer((_) async => http.Response('Error', 404));

          expect(() => httpClient.getBytes('test'), throwsException);
        },
      );
    });
    ...
    group('Timeout Handling', () {
      test(
        'request times out after specified duration',
        () async {
