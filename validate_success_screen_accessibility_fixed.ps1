...
if ($content -match "image:\s*true" -and $content -match "label:\s*.*Erfolgreich") {
...
if ($content -match "header:\s*true" -and $content -match "container:\s*true") {
...
if ($content -match "Colors\.green" -and $content -match "Colors\.black87") {
...
# 2.1.1 Keyboard
if ($content -match "FocusNode" -and $content -match "onKeyEvent") {
...
if ($content -match "dispose" -and $content -match "requestFocus") {
    Write-Host "✅ 2.1.2 No Keyboard Trap: Proper focus management" -ForegroundColor Green
    $score++
} else {
...
# 2.4.1 Bypass Blocks
if ($content -match "BaseScreenLayoutAccessible") {
    Write-Host "✅ 2.4.1 Bypass Blocks: Skip navigation available" -ForegroundColor Green
    $score++
} else {
...
# 2.4.2 Page Titled
if ($content -match "E-Mail-Bestätigung erfolgreich") {
...
# 2.4.3 Focus Order
if ($content -match "_setInitialFocus" -and $content -match "addPostFrameCallback") {
...
if ($content -match "actionButtonHint" -and $content -match "hint:") {
...
if ($content -match "TextDirection\.ltr" -and $content -match "deutsch") {
    Write-Host "✅ 3.1.1 Language: German language specified" -ForegroundColor Green
    $score++
} else {
...
if ($content -match "Tipp:" -or $content -match "Verwenden Sie Tab") {
...
# 4.1.1 Parsing
if ($content -match "extends StatefulWidget" -and $content -match "@override") {
...
if ($content -match "button:\s*true" -and $content -match "label:") {
...
# 2.4.7 Focus Visible
if ($content -match "BorderSide" -and $content -match "hasFocus") {
...
Write-Host ("📊 FINAL BITV 2.0 COMPLIANCE SCORE: {0}/{1} ({2}%)" -f $score, $total, $percentage) -ForegroundColor Cyan
...
if ($percentage -ge 95) {
    ...
    Write-Host "❌ RATING: UNGENUEGEND (Inadequate) - Major accessibility barriers" -ForegroundColor Red
}
