...
class AgbScreenAccessible extends StatelessWidget {
  const AgbScreenAccessible({super.key});
...
''';
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      container: true,
      label: 'Allgemeine Geschäftsbedingungen Bildschirm',
      hint:
          'Zeigt die rechtlichen Bedingungen für Schulungen und Kurse des BSSB an',
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Semantics(
            ...
            child: const Text('AGB', style: UIStyles.appBarTitleStyle),
          ),
          backgroundColor: UIConstants.backgroundColor,
          ...
          leading: Semantics(
            ...
            hint: 'Schließt die AGB und kehrt zur vorherigen Seite zurück',
            child: IconButton(
        ...
        body: Semantics(
          container: true,
          label: 'AGB Dokumentinhalt',
          hint:
              'Scrollbarer Bereich mit allen Geschäftsbedingungen in strukturierten Abschnitten',
          child: Center(
              ...
              child: Container(
                ...
                child: Builder(
                  builder: (context) {
                    ...
                    return Semantics(
                      container: true,
                      label: 'AGB Strukturierter Inhalt',
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          // Document title with enhanced accessibility
                          if (mainSections.isNotEmpty &&
                              mainSections.first.title != null &&
                              mainSections.first.title!.contains(
                                  'Allgemeine Geschäftsbedingungen',)) ...[
                            Semantics(
                              header: true,
                              label:
                                  'Dokumenttitel: ${mainSections.first.title}',
                              child: Text(
                                mainSections.first.title!,
                          ...
                          // Process all sections with enhanced semantics
                          for (int sectionIndex = 0;
                              sectionIndex < mainSections.length;
                              sectionIndex++) ...[
                            _buildAccessibleSection(mainSections[sectionIndex],
                                sectionIndex, mainSections.length,),
                            if (sectionIndex < mainSections.length - 1)
                              UIConstants.verticalSpacingM,
                          ],
                          // Document footer with accessibility
                          if (footer != null)
                            Semantics(
                              container: true,
                              label: 'Dokumentdatum: $footer',
                              hint: 'Gültigkeitsdatum der Geschäftsbedingungen',
                              child: Padding(
        ...
        floatingActionButton: Semantics(
          ...
          label: 'AGB schließen',
          hint:
              'Schließt das AGB-Dokument und kehrt zur vorherigen Seite zurück',
          child: FloatingActionButton(
            onPressed: () => Navigator.of(context).pop(),
            backgroundColor: UIConstants.defaultAppColor,
            tooltip: 'AGB schließen',
            child: Semantics(
  ...
  Widget _buildAccessibleSection(
      _AgbSection section, int sectionIndex, int totalSections,) {
    return Semantics(
      container: true,
      label: section.title != null
          ? 'Abschnitt ${sectionIndex + 1} von $totalSections: ${section.title}'
          : 'Abschnitt ${sectionIndex + 1} von $totalSections',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Section title with enhanced accessibility
          if (section.title != null &&
              !section.title!.contains('Allgemeine Geschäftsbedingungen')) ...[
            Semantics(
              header: true,
              label: 'Abschnittstitel: ${section.title}',
              child: Text(
                section.title!,
          ...
          // Section paragraphs with enhanced accessibility
          for (int paraIndex = 0;
              paraIndex < section.paragraphs.length;
              paraIndex++) ...[
            _buildAccessibleParagraph(section.paragraphs[paraIndex], paraIndex,
                section.paragraphs.length, section.title,),
            if (paraIndex < section.paragraphs.length - 1)
  ...
  Widget _buildAccessibleParagraph(
      String para, int paraIndex, int totalParas, String? sectionTitle,) {
    final isNumbered = RegExp(r'^\d+(?:\.\d+)*\.\s*').hasMatch(para) &&
        !para.startsWith('Stand:');

    return Semantics(
      container: true,
      label: isNumbered
          ? 'Nummerierter Absatz ${paraIndex + 1} von $totalParas'
          : 'Absatz ${paraIndex + 1} von $totalParas',
      hint: sectionTitle != null ? 'Teil von Abschnitt: $sectionTitle' : null,
      child: isNumbered
          ? _buildAccessibleNumberedParagraph(para)
          : _buildAccessibleRegularParagraph(para),
    );
  }

  Widget _buildAccessibleNumberedParagraph(String para) {
    ...
    if (match != null) {
      ...
      return Semantics(
        container: true,
        label: 'Rechtlicher Punkt $numbers: $text',
        child: Text.rich(
          TextSpan(
            children: [
              TextSpan(
    ...
    return _buildAccessibleRegularParagraph(para);
  }

  Widget _buildAccessibleRegularParagraph(String para) {
    // Check for special paragraph types
    String label = 'Textabsatz';
    String? hint;

    if (para.contains('Widerrufsrecht') || para.contains('Widerrufsfolgen')) {
      label = 'Wichtiger Hinweis zu Widerrufsrechten';
      hint = 'Rechtliche Information zu Ihren Widerrufsrechten';
    } else if (para.contains('SEPA') || para.contains('Lastschrift')) {
      label = 'Zahlungsbestimmungen';
      hint = 'Information zu Zahlungsmodalitäten und Bankeinzug';
    } else if (para.contains('Gerichtsstand') ||
        para.contains('anwendbares Recht')) {
      label = 'Rechtliche Bestimmungen';
      hint =
          'Gerichtsstand und anwendbares Recht für rechtliche Streitigkeiten';
    }
